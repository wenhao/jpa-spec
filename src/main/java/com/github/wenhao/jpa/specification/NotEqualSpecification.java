...
public class NotEqualSpecification<T> implements Specification<T>, Serializable {
    ...
    public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
        ...
        Arrays.asList(Arrays.copyOfRange(values, 0, values.length - 2)).forEach(value -> criteriaBuilder.or(criteriaBuilder.notEqual(root.get(property), values[values.length - 1])));
